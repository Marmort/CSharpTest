    ...
    public class Hero
	{
	    ...
	    public bool Save()
	    {
	        try
	        {
                SQLiteCommand command = new SQLiteCommand(string.Format(
                            "UPDATE hero SET experience='{0}', level='{1}' WHERE hero_id='{2}'", 
                            Experience, 
                            Level, 
                            Id), 
	    ...
	    public bool Create(int accountId)
	    {
            ...
            if (CheckIfHeroExists(accountId, Name))
                return false;
            SQLiteCommand command = new SQLiteCommand(string.Format(
                        "INSERT INTO hero (account_id, name, hero_class_id, hero_gender_id, experience, level) VALUES('{0}','{1}','{2}','{3}', '{4}', '{5}')", 
                        accountId, 
                        Name, 
                        HeroClass, 
                        Gender, 
	    ...
	    {
            SQLiteCommand command = new SQLiteCommand(string.Format(
                        "SELECT hero_id FROM hero WHERE account_id='{0}' AND name='{1}'", 
                        account_id, 
                        hero_name), 
                        Database.Instance.Connection);
	    ...
	    {	        
            hero = null;
            SQLiteCommand command = new SQLiteCommand(string.Format(
                        "SELECT hero_id,account_id, name, hero_class_id, hero_gender_id, experience, level FROM hero WHERE hero.hero_id='{0}'", 
                        id), 
                        Database.Instance.Connection);
            SQLiteDataReader reader = command.ExecuteReader();
	        if (reader.HasRows)
